...
static unsigned from_utf8(unsigned uni_code) {
    const unsigned int *page = from_uni[(uni_code >> 8) & 0xFF];
    return page == NULL ? DBCS_DEFAULT_CODE : page[uni_code & 0xFF];
}

static unsigned to_utf8(unsigned cp_code) {
    std::map<unsigned int, unsigned int>::const_iterator itr = to_uni.find(cp_code);
    if(itr != to_uni.end())
        return itr->second;
    return UNI_INVALID_CODE;
}
...
static size_t dbcs_decode(const char *s, const char *e, unsigned *pch) {
    ...
    ch = (us[0] << 24) | (us[1] << 16) | (us[2] << 8) | us[3];
    std::map<unsigned int, unsigned int>::const_iterator itr = to_uni.find(ch);
    if(itr != to_uni.end()) {
        *pch = ch;
        return 4;
    }
...
static void add_dbcschar(luaL_Buffer *b, unsigned ch) {
    if (ch < 0x7F)
        ...
        luaL_addchar(b, (ch >> 24) & 0xFF);
        luaL_addchar(b, (ch >> 16) & 0xFF);
        luaL_addchar(b, (ch >> 8) & 0xFF);
        luaL_addchar(b, ch & 0xFF);
    }
}
